...
tasksTable = pd.read_csv('Production Needs3.csv', sep=",", header=0)
...
with open('UnassignTaskType.csv', 'w', encoding='UTF8') as f:
        ...
        for t in time_window:
          ...
          for k in TaskList:
            if ((any(employee_task_vars[e,k].solution_value>0 for e in employeelist)) and (task_room[k]==Room)):
              case+=1
    if case>0:
      writer.writerow([i,'I','The task can not be done due to room limitation',Room])
      deletelist.append(i)
  ...
  for i in newunassignlist:
    ...
    for j in EmployeeList:
      ...
      index.append(case)
    if sum(index)>0:
      writer.writerow([i,'II','Task can be done if employee work longer time'])
    else:
      writer.writerow([i,'III','We should tran more employee',Skill])
