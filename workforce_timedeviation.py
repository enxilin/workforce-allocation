...
## store all ...with docpelx,path
root = tkinter.Tk()
root.withdraw()
#Folderpath=filedialog.askdirectory()# ask for where you store the csv for employee' skill, avilibility
Filename = filedialog.askopenfilename()#a sk for selecting production need document in xlsx form.
#os.chdir(Folderpath)
...
# replace the file name with testfile when you actually use 
...
for e in employeelist:
  for s in studylist:
    ...
    mdl.add_constraint(study_number[e,s]<=number[e,s]*nb_task)
