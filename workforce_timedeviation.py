...
selected_columns = tasksTable[["Need ID","Time","SkillsetID1","Allowble Negative Deviation (hrs)"]]
...
for i in range(nb_task):
    ...
    for j in range(1,minus_num+1):
      ...
      'Time':[timelist[taskstart-j:taskend-j]],'Room':[tasksTable.iloc[i,9]]})
...
for i in unassignlist:
  ...
  Deviation=assignTable.loc[assignTable["Need ID"]==i]["Allowble Negative Deviation (hrs)"].to_string(index=False)
  ...
  for j in EmployeeList:
    ...
    index.append(case)
  if sum(index)>0:
    print("task",i,'can be done if employee work overtime.')
  else:
    print("we should train skill",Skill,"for task",i)
