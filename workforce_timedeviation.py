...
def EmployeeTime(x):
  ...
  for i in range(len(x)):
    ...
    x.iat[i,1]=list(range(48)[starttime:lunchs])+list(range(48)[lunche:endtime])
  employee_time={}
...
for i in range(nb_task):
    ...
    for j in range(1,minus_num+1):
      x_new_row=pd.DataFrame({'Need ID':[(tasksTable.iloc[i,0],"-",j)],'SkillsetID1':[tasksTable.iloc[i,2]],'Time':[list(range(48)[taskstart-j:taskend-j])]})
...
for e in employeelist:
  ...
  work_time.append(max(end_time-start_time+1,0))
  print(work_time)
...
def createLabels(ax, title, xlabel, ylabel):
  tmin=int(min(employee_time.values())[0])/2
  tmax=int(max(employee_time.values())[-1]+1)/2
