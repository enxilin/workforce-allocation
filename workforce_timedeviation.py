...
task_study=tasksTable.set_index('Need ID').to_dict()['Study Number']
...
employee_study_var=mdl.continuous_var_matrix(employeelist,tasklist)
study_binary=mdl.binary_var_cube(employeelist,nb_tasks,nb_tasks)
study_number=mdl.binary_var_matrix(employeelist,nb_tasks)

for e in employeelist:
  for t in tasklist:
    mdl.add_constraint(employee_study_var[e,t]==employee_task_vars[e,t]*ord(task_study[t]))


for e in employeelist:
  for i in range(0, nb_tasks):
    for j in range(i+1, nb_tasks):
      s1=tasksTable.iloc[i,0]
      s2=tasksTable.iloc[j,0]
      mdl.add_constraint(study_binary[e,i,j]<= mdl.abs(employee_study_var[e,s1] - employee_study_var[e,s2]))
      mdl.add_constraint(study_binary[e,i,j]*68 >= mdl.abs(employee_study_var[e,s1] - employee_study_var[e,s2]))
    mdl.add_constraint((1-study_number[e,i]) * 90 >= (nb_tasks-i-1)-mdl.sum(study_binary[e,i,j]for j in range(i+1,nb_tasks)))
    mdl.add_constraint((1-study_number[e,i])<= (nb_tasks-i-1)-mdl.sum(study_binary[e,i,j]for j in range(i+1,nb_tasks)))
study_varibility=mdl.sum(study_number[e,i] for e in employeelist for i in range(0,nb_tasks)) - len(avilibilityTable)
...
mdl.add_kpi(study_varibility,'study_varibility')
mdl.print_information()

##Aim function and slove
mdl.maximize(10*total_number_of_assignments-0.1*total_work-0.1*study_varibility)
s = mdl.solve()
