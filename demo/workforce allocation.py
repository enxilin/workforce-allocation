...
#  Info: Constraint 7: tasks predecessors
...
mdl.add_kpi(total_study_varibility,'total study varibility')



###Aim function and slove
mdl.maximize(10*total_number_of_assignments-0.1*total_study_varibility-0.1*total_work)
s = mdl.solve()
mdl.report()
...
def displayTasksAssignmentsGantt(ax):
  ...
  for i in range(len(employeelist)):
    ...
    ax.bar(lunchs+0.5,0.3,employee_time[n][-1]-lunchs,bottom=i+0.5,color='k',align='edge')
    for s in tasklist:
      if employee_task_vars[n,s].solution_value >0.5:
  ...
  ax.set_ylabel('Employee ID & Skill')
        ...
        if task_time[s][0]==t:
          for e in employeelist:
            if employee_task_vars[e,s].solution_value > 0:
              j+=0.2
              ax.bar(task_time[s][0], 0.2,width=task_time[s][-1] - task_time[s][0]+0.455, bottom=i + j,
              color=colorBySkills[task_skill[s]],align='edge')
              ax.text(task_time[s][0],i+j,s[0],fontsize=12)              
    ylabels.append("{}:{}".format('Room',str(n))) # (3)number of total hours works
    tickloc.append(i + 0.2)
    plt.axhline(y= i+0.2, color = '0.8')
  tmin=int(min(employee_time.values())[0])
  tmax=math.ceil(int(max(employee_time.values())[-1])+0.5)
  for i in range(tmin,tmax+1):
    plt.axvline(x=i, color = '0.8')
...
def unassignTask(assignlist,o_tasklist):
  for e in employeelist:
    for t in tasklist:
      if employee_task_vars[e,t].solution_value > 0.5:
  ...
  for i in unassignlist:
    ...
    if (np.isnan(Predecessor)==False):
      if int(Predecessor) in unassignlist:
        ...
        deletelist.append(i)
  newunassignlist=list(set(unassignlist)-set(deletelist))
  deletelist.clear()  
    ...
    for j in range(-1*int(nb_negative),int(nb_positive)+1): # each allowable time deviation of the unassigned task:
      ...
      if any(len(list(set(time_window).intersection(employee_time[e])))==len(task_time[(i,"0")]) for e in EmployeeList):
        ...
        for t in time_window:
          ...
          for k in TaskList:
            if ((any(employee_task_vars[e,k].solution_value>0.5 for e in employeelist)) and (task_room[k]==Room)):
              case_t+=1
          if case_t>=2:
            case_T+=1
    if case_T>0:
      ...
      deletelist.append(i)
  newunassignlist=list(set(newunassignlist)-set(deletelist))
